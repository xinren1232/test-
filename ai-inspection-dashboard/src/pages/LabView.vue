    ...
    <el-row :gutter="20" class="charts-container">
      <el-col :span="24">
        <el-card shadow="hover" class="material-ng-chart-card">
          ...
          <!-- 数据分析统计区域 -->
          <el-row :gutter="20" class="ng-statistics-summary">
            <el-col :xs="12" :sm="6">
              <div class="stat-box">
                <div class="stat-title">总批次数</div>
                <div class="stat-value primary-stat">{{ ngAnalysisData.totalBatchCount }}</div>
                <div class="stat-trend" :class="{'trend-up': parseFloat(ngAnalysisData.totalBatchPercentage) > 0, 'trend-down': parseFloat(ngAnalysisData.totalBatchPercentage) < 0}">
                  {{ parseFloat(ngAnalysisData.totalBatchPercentage) > 0 ? '↑' : '↓' }} {{ ngAnalysisData.totalBatchPercentage }}
              </div>
              </div>
            </el-col>
            <el-col :xs="12" :sm="6">
              <div class="stat-box">
                <div class="stat-title">不良数量</div>
                <div class="stat-value danger-stat">{{ ngAnalysisData.ngMaterialCount }}</div>
                <div class="stat-trend" :class="{'trend-up': parseFloat(ngAnalysisData.ngMaterialPercentage) > 10, 'trend-down': parseFloat(ngAnalysisData.ngMaterialPercentage) < 10}">
                  不良率: {{ ngAnalysisData.ngMaterialPercentage }}
              </div>
            </div>
      </el-col>
            <el-col :xs="12" :sm="6">
              <div class="stat-box">
                <div class="stat-title">最高不良供应商</div>
                <div class="stat-value warning-stat">{{ ngAnalysisData.worstSupplier }}</div>
                <div class="stat-trend trend-up" v-if="ngAnalysisData.worstSupplier !== 'N/A'">
                  不良率: {{ ngAnalysisData.worstSupplierRate }}%
            </div>
            </div>
            </el-col>
            <el-col :xs="12" :sm="6">
              <div class="stat-box">
                <div class="stat-title">主要不良现象</div>
                <div class="stat-value info-stat">{{ ngAnalysisData.topDefect }}</div>
                <div class="stat-trend trend-up" v-if="ngAnalysisData.topDefect !== 'N/A'">
                  占比: {{ ngAnalysisData.topDefectPercentage }}%
                </div>
                  </div>
            </el-col>
          </el-row>
          ...
          <div class="chart-actions">
            <el-button type="primary" size="small" @click="generateNgReport">
              <el-icon><Document /></el-icon> 生成分析报告
            </el-button>
            <el-button type="warning" size="small" @click="showNgDetails">
              <el-icon><View /></el-icon> 查看详细数据
            </el-button>
                  </div>
        </el-card>
      </el-col>
...
import { getBaselineByProject } from '../data/ProjectBaselineMap.js';
import { getMaterialCode } from '../data/MaterialCodeMap.js';
...
const updateMaterialNgBySupplierChart = () => {
  ...
  ngTests.forEach(item => {
    ...
    const defectDesc = item.defectDesc || '未知不良';
    
    // 创建以物料名称为键的对象
    if (!materialSupplierStats[materialName]) {
      materialSupplierStats[materialName] = {
        name: materialName,
        totalCount: 0,
        suppliers: {},
        defects: {}
    ...
    // 增加该物料的总计数
    materialSupplierStats[materialName].totalCount++;
    
    // 增加该物料对应供应商的计数
    if (!materialSupplierStats[materialName].suppliers[supplier]) {
      materialSupplierStats[materialName].suppliers[supplier] = 0;
    }
    materialSupplierStats[materialName].suppliers[supplier]++;
    
    // 增加该物料对应不良现象的计数
    if (!materialSupplierStats[materialName].defects[defectDesc]) {
      materialSupplierStats[materialName].defects[defectDesc] = 0;
    }
    materialSupplierStats[materialName].defects[defectDesc]++;
  });
  
  // 转换为数组并按不良总数排序（降序）
  const materialData = Object.values(materialSupplierStats)
    .sort((a, b) => b.totalCount - a.totalCount)
    .slice(0, 10); // 最多显示前10种物料
  
  // 获取所有物料的供应商列表
  const allSuppliers = new Set();
  materialData.forEach(material => {
    Object.keys(material.suppliers).forEach(supplier => {
  ...
  const seriesData = supplierList.map(supplier => {
    const data = materialData.map(material => {
      return material.suppliers[supplier] || 0;
    });
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function(params) {
        const materialName = params[0].axisValue;
        const material = materialData.find(item => item.name === materialName);
        
        if (!material) return '';
        
        let tooltip = `<div style="font-weight:bold;font-size:14px;margin-bottom:5px">${materialName}</div>`;
        tooltip += `<div style="margin-bottom:5px">不良总数: <b>${material.totalCount}</b></div>`;
        
        // 供应商分布
        tooltip += '<div style="font-weight:bold;margin-top:5px;border-bottom:1px solid #ccc;padding-bottom:3px">供应商分布:</div>';
        
        // 获取供应商数据并排序
        const supplierData = Object.entries(material.suppliers)
          .sort((a, b) => b[1] - a[1]);
        
        supplierData.forEach(([supplier, count]) => {
          const percentage = ((count / material.totalCount) * 100).toFixed(1);
          tooltip += `<div>${supplier}: ${count}件 (${percentage}%)</div>`;
        });
        
        // 不良现象分布
        tooltip += '<div style="font-weight:bold;margin-top:8px;border-bottom:1px solid #ccc;padding-bottom:3px">常见不良现象:</div>';
        
        // 获取不良现象数据并排序，只显示前3个
        const defectData = Object.entries(material.defects)
          .sort((a, b) => b[1] - a[1])
          .slice(0, 3);
        
        defectData.forEach(([defect, count]) => {
          const percentage = ((count / material.totalCount) * 100).toFixed(1);
          tooltip += `<div>${defect}: ${count}件 (${percentage}%)</div>`;
        });
...
const loadLabTestData = async () => {
  try {
    ...
    testData.value = labData.map(item => ({
      ...
      materialCode: getMaterialCode(item.materialName, item.supplier) || item.materialCode || item.material_id || '未知编码',
      ...
      baseline: getBaselineByProject(item.project_id)?.baseline_name || item.baseline_name || item.baseline_display || item.baselineName || item.baselineId || item.baseline_id || '未关联基线',
...
// 按不良现象分析图表 - 此功能已整合到物料供应商图表中
const updateDefectNgChart = () => {
  // 这个函数是为了保持接口一致，现在所有分析都在updateMaterialNgBySupplierChart中处理
  console.log("不良现象分析已整合到物料供应商分布图表中");
};
