...
async function testDatabaseQueries() {
  try {
    ...
    const [labTestCount] = await connection.execute('SELECT COUNT(*) as count FROM lab_tests');
    const [onlineTrackingCount] = await connection.execute('SELECT COUNT(*) as count FROM online_tracking');
    const [testTrackingCount] = await connection.execute('SELECT COUNT(*) as count FROM test_tracking');

    console.log(`📦 库存记录总数: ${inventoryCount[0].count}`);
    console.log(`🧪 实验室测试记录总数: ${labTestCount[0].count}`);
    console.log(`📊 在线跟踪记录总数: ${onlineTrackingCount[0].count}`);
    console.log(`📋 测试跟踪记录总数: ${testTrackingCount[0].count}`);
