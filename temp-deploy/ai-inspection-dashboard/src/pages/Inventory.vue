  ...
  setup() {
    ...
    watch([searchQuery, statusFilter, warehouseFilter, inventoryData], () => {
      ...
      if (searchQuery.value) {
        const query = searchQuery.value.toLowerCase();
        data = data.filter(item => 
          (item.materialCode && String(item.materialCode).toLowerCase().includes(query)) ||
          (item.materialName && String(item.materialName).toLowerCase().includes(query)) ||
          (item.supplier && String(item.supplier).toLowerCase().includes(query)) ||
          (item.batchNo && String(item.batchNo).toLowerCase().includes(query)) // <-- Bug修复在这里！
        );
      }
